...
class TransformerClassifier(nn.Module):
    def __init__(self, vocab_size, embed_dim, num_heads, num_classes, num_layers, max_len=512):
        super(TransformerClassifier, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embed_dim)
        self.positional_encoding = nn.Parameter(torch.zeros(1, max_len, embed_dim)) # 초기 최대 길이
        encoder_layer = nn.TransformerEncoderLayer(embed_dim, num_heads, dim_feedforward=512, dropout=0.1)
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
        self.fc = nn.Linear(embed_dim, num_classes)

    def forward(self, x):
        embedded = self.embedding(x) + self.positional_encoding[:, :x.size(1), :]   # Add positional encoding
        transformer_out = self.transformer(embedded.permute(1, 0, 2))   # (seq_len, batch_size, embed_dim)
        cls_token = transformer_out[0]  # Use the first token as classification token
